...
func (h *CMSUserHandler) Index(c *gin.Context) {
	...
	c.HTML(http.StatusOK, "cms/user_list.html", withUserCtx(c, gin.H{
		"title": "Manajemen Pengguna", "active_menu": "users",
		"users": users, "flash": c.Query("flash"),
	}))
}
...
func (h *CMSUserHandler) CreatePage(c *gin.Context) {
	...
	c.HTML(http.StatusOK, "cms/user_form.html", withUserCtx(c, gin.H{
		"title": "Tambah Pengguna", "active_menu": "users",
		"roles": roles, "is_edit": false,
	}))
}
...
func (h *CMSUserHandler) Create(c *gin.Context) {
	...
	if selectedRole == nil {
		c.HTML(http.StatusOK, "cms/user_form.html", withUserCtx(c, gin.H{
			"title": "Tambah Pengguna", "active_menu": "users",
			"roles": roles, "is_edit": false, "error_info": "Role yang dipilih tidak valid",
		}))
		return
	}
	...
	if err != nil {
		c.HTML(http.StatusOK, "cms/user_form.html", withUserCtx(c, gin.H{
			"title": "Tambah Pengguna", "active_menu": "users",
			"roles": roles, "is_edit": false, "error_info": "Gagal membuat password acak, coba lagi",
		}))
		return
	}
	...
	if err != nil {
		c.HTML(http.StatusOK, "cms/user_form.html", withUserCtx(c, gin.H{
			"title": "Tambah Pengguna", "active_menu": "users",
			"roles": roles, "is_edit": false, "error_info": "Gagal membuat akun, coba lagi",
		}))
		return
	}

	newUser := &domain.User{
		Name: name, Email: email, Password: string(hash),
		IsActive: true, RoleID: uint(roleID), MustChangePassword: true,
	}
...
func (h *CMSUserHandler) EditPage(c *gin.Context) {
	...
	roles, _ := h.userRepo.FindAllRoles()
	c.HTML(http.StatusOK, "cms/user_form.html", withUserCtx(c, gin.H{
		"title": "Edit Pengguna", "active_menu": "users",
		"user": user, "roles": roles, "is_edit": true,
	}))
}
...
func (h *CMSUserHandler) Delete(c *gin.Context) {
	...
	avatar := user.Avatar // simpan sebelum hapus
	h.userRepo.Delete(uint(id))
	removeFile(avatar) // hapus foto profil dari disk
	c.Redirect(http.StatusFound, "/cms/users?flash=deleted")
}
