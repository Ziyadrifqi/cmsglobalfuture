...
interface TeamMember  { icon: string; name: string; role: string }
interface ValueItem   { icon: string; title: string; desc: string }
interface Milestone   { year: string; label: string }

const FALLBACK_TEAM: TeamMember[] = [
  { icon: '👨‍💼', name: 'Rizal Anwar, M.Env', role: 'Ketua Umum' },
  { icon: '👩‍🌾', name: 'Nadya Putri, S.Hut', role: 'Kepala Program Lapangan' },
  { icon: '👨‍💻', name: 'Bayu Santoso',       role: 'Kepala Komunikasi & Media' },
  { icon: '👩‍💼', name: 'Sari Dewi, M.Si',   role: 'Manajer Relawan' },
]
...
const FALLBACK_MISSION = [
  'Menggerakkan aksi restorasi lingkungan berbasis komunitas',
  'Membangun jaringan relawan lingkungan di seluruh Indonesia',
  'Mendorong kebijakan lingkungan yang berpihak pada alam',
  'Mengedukasi generasi muda tentang krisis iklim',
  'Berkolaborasi dengan pemerintah dan sektor swasta',
]
...
export function AboutPage() {
  const { data: about, isLoading: loadAbout } = useQuery({
    queryKey: ['page', 'about'],
    queryFn:  () => portalApi.page('about'),
  })
  const { data: settings } = useSettings()

  const visionText    = getText(settings, 'about_vision_text',
  ...
  const missionItems  = getJSON<string[]>(settings, 'about_mission_items', FALLBACK_MISSION)
    ...
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="bg-white rounded-2xl border border-teal-100 shadow-sm p-8 mb-6">
        ...
        {loadAbout ? <SkeletonBlock /> : (
          <div className="text-slate-600 leading-[1.9] text-[15px] whitespace-pre-wrap">
            {about?.Content ?? FALLBACK_ABOUT}
          </div>
        )}
      </div>

      {/* Visi & Misi — sekarang dari settings */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        <div className="bg-gradient-to-br from-[#134E4A] to-[#166534] rounded-2xl p-8 text-white">
          ...
          <p className="text-teal-100 leading-relaxed text-sm">{visionText}</p>
        </div>
          ...
          {milestones.map((m, i) => (
            <div key={m.year + i} className="flex gap-5 pb-6 last:pb-0">
              <div className="flex flex-col items-center">
                <div className="w-10 h-10 rounded-full bg-teal-600 text-white flex items-center justify-center text-xs font-black shrink-0">
                  {m.year.slice(2)}
                </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {team.map(m => (
          <div key={m.name} className="bg-white rounded-2xl p-6 border border-teal-100 text-center hover:shadow-md hover:-translate-y-0.5 transition-all">
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-teal-100 to-emerald-200 flex items-center justify-center text-3xl mx-auto mb-4">
              {m.icon}
            </div>
